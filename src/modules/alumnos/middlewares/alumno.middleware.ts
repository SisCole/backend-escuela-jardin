...
import { alumnoValidationSchema, alumnoParamsValidationSchema } from '../validators/alumno.validator';
...
export function validateAlumnoParams(req: AlumnoRequest, res: Response, next: NextFunction) {
    const { error, value } = alumnoParamsValidationSchema.validate(req.params, { abortEarly: false });
    if (error) {
        return res.status(400).json({ error: error.details.map(detail => detail.message) });
    }
    req.params = value;
